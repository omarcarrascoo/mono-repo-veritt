...
export default function FaiOpeningScreen() {
  ...
  const draft = useFaiDraft({
    ...
    materials,
    ready: !isLoading && !!selectedLocation,
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (locOptions.length > 0 && !selectedLocation) {
          // Preferir la ubicación primaria si existe
  ...
  const jumpFromReview = useCallback(
    (materialId: string) => {
      setReviewOpen(false);
      // pequeño delay para evitar animaciones encimadas
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      await dailyChainApi.createOpening(businessId, {
        locationId: selectedLocation,
        items: counted.map((i) => ({
          materialId: i.materialId,
          countedQuantity: i.counted as number,
          varianceNote: buildVarianceNote(i),
        })),
      });
      ...
      notify.success(
        'Conteo enviado',
        'Un gerente debe autorizarlo para abrir el día.',
      );
  ...
  if (materials.filter((m) => m.status === 'ACTIVE').length === 0) {
    return (
      <View style={{ flex: 1, backgroundColor: surface.paper }}>
        ...
        <VrittScreenHeader
          onBack={onBack}
          title="Apertura"
          eyebrow="FAI · Conteo inicial"
        />
        <View style={{ flex: 1, padding: 22, justifyContent: 'center', gap: 14, alignItems: 'center' }}>
          <Ionicons
            name="cube-outline"
            size={32}
          ...
          >
            Da de alta materiales en inventario para abrir el día.
          </Text>
        </View>
      </View>
    );
    ...
    >
      ...
      <View style={{ gap: 8 }}>
        ...
        >
          ...
          >
            {variance > 0 ? (
              <DotLabel color={palette.amber} value={variance} label="varianza" />
            ) : null}
            {skipped > 0 ? (
              <DotLabel color={palette.danger} value={skipped} label="saltado" />
            ) : null}
          </View>
        </View>
      </View>
      ...
      >
        <View style={{ gap: 4, flex: 1, minWidth: 0 }}>
          ...
          >
            ...
            {skipped > 0 ? ` · ${skipped} saltado${skipped === 1 ? '' : 's'}` : ''}
          </Text>
          <Text
            style={{
